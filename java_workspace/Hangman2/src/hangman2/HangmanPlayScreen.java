...
public class HangmanPlayScreen extends JFrame implements ActionListener {
	...
	public HangmanPlayScreen(String[] gameWords, String[] wordDefinitions, String theme, String gameLevel) {
		...
		for (int i = 0; i < gameWords.length; i++) {
			set.put(gameWords[i].trim(), wordDefinitions[i].trim());
		}
		
		for (int i = 0; i < gameWords.length; i++) {
			list.add(gameWords[i].trim());
			System.out.println(list.get(i));
		}
		
		Collections.shuffle(list);
		for (int i = 0; i < list.size(); i++) {
			System.out.println(list.get(i) + " " + set.get(list.get(i)));
		}
		...
		word = list.get(questionNum[questionCnt]);
		System.out.println("word : "+word);

		// wordDefinitions[인덱스 번호 0] 부터 시작한다
		wordMeaning = set.get(word);
		...
		next.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (questionCnt < random.size()) {
					...
					System.out.println();
				} else if (questionCnt == random.size()) { // questionNum이 단어 개수와 같을때 questionNum과 hintNum을 0으로 초기화
...
//					totalRecord += playerRecord + "\n";
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (hintNum >= 0) {
					...
					if (total.equalsIgnoreCase(totalLine)) {
						...
						score.setText("SCORE " + scoreCnt + "/" + list.size());
						if (scoreCnt > 0) {
	...
	public void setScorePanel() {
		// 점수를 보여주는 패널 배치
		score = new JLabel("SCORE " + scoreCnt + "/" + list.size());
